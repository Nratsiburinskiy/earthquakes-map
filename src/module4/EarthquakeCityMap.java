...
fill(255, 250, 240);
	...
	private void printQuakes() 
	{
		...
		int countOceanMarker = 0;
		boolean flag=true;
		for(Marker cm : countryMarkers) {
			int countLandMarker = 0;
			String name = (String) cm.getProperty("name");
			for(Marker qm : quakeMarkers) {
				EarthquakeMarker em = (EarthquakeMarker)qm;
				if(em.isOnLand()) {
					String c = (String) em.getProperty("country");
					if(c.equals(name)) {
						countLandMarker = countLandMarker + 1;
					}
				}
				else {
					if(flag) {
						countOceanMarker = countOceanMarker + 1;
			...
			flag =false;
			if(countLandMarker >= 1) {
				System.out.println(name + ": "+ countLandMarker);
				}
			
		}
		System.out.println("Ocean Quakes: " + countOceanMarker);
